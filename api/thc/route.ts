...
export async function POST(req: Request) {
  try {
    ...
    const { action, path: filePath, content, command, dir, name, data } = body;
    ...
    switch (action) {
      ...
      case 'reboot': {
        const rebootCommand = 'npm run build && (fuser -k 3011/tcp; PORT=3011 nohup npm run start > 3011.log 2>&1 &)';
